...
static jl_value_t *ulong JL_ALWAYS_LEAFTYPE = NULL;
...
static jl_array_t *jl_verify_edges(jl_array_t *targets, size_t minworld)
{
    ...
    if (ulong == NULL)
        ulong = jl_apply_array_type((jl_value_t*)jl_ulong_type, 1);
    jl_array_t *maxvalids = jl_alloc_array_1d(ulong, l);
...
static jl_array_t *jl_verify_methods(jl_array_t *edges, jl_array_t *maxvalids)
{
    ...
    size_t *maxvalids2_data = jl_array_data(maxvalids2);
